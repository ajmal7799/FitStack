...
export class VideoCallRepository extends BaseRepository<VideoCall, IVideoCallModel> implements IVideoCallRepository {
  ...
  async findSessionsByUserId(userId: string, skip: number, limit: number): Promise<VideoCall[]> {
  ...
  async findSessionsByTrainerId(trainerId: string, skip: number, limit: number, search?: string): Promise<VideoCall[]> {
    const matchStage: FilterQuery<IVideoCallModel> = {
    ...
    const pipeline: PipelineStage[] = [
      ...
      ...(search?.trim() ? [{ $match: { 'user.name': { $regex: search.trim(), $options: 'i' } } }] : []),
      { $sort: { startTime: -1 } },
      { $skip: skip },
      { $limit: limit },
    ];
    ...
    return results.map(videoCall => VideoCallMapper.fromMongooseDocument(videoCall));
  }

  async countSessionsByTrainerId(trainerId: string, search?: string): Promise<number> {
    const matchStage: FilterQuery<IVideoCallModel> = {
      trainerId: new Types.ObjectId(trainerId),
      status: VideoCallStatus.COMPLETED,
    };
  ...
  async findAllBookedSessionByUserId(
    userId: string,
    skip: number,
    limit: number,
  ...
  async findAllBookedSessionByTrainerId(
    trainerId: string,
    skip: number,
    limit: number,
    status?: VideoCallStatus,
    ...
    const matchStage: FilterQuery<IVideoCallModel> = {
      trainerId: new Types.ObjectId(trainerId), // ✅ fixed
    };
    if (status) {
    ...
    const videoCalls = await this._model.aggregate(pipeline);
    return videoCalls.map(videoCall => VideoCallMapper.fromMongooseDocument(videoCall));
  }

  async countBookedSessionByTrainerId(trainerId: string, status?: VideoCallStatus, search?: string): Promise<number> {
    const matchStage: FilterQuery<IVideoCallModel> = {
      trainerId: new Types.ObjectId(trainerId), // ✅ fixed
    };
    if (status) {
  ...
  async findByIds(ids: string[]): Promise<VideoCall[]> {
    const objectIds = ids.map(id => new Types.ObjectId(id));
    const docs = await this._model.find({ _id: { $in: objectIds } });
    return docs.map(doc => VideoCallMapper.fromMongooseDocument(doc));
  }
}
